...
def create_dict_features(which_dataset, original_dataset_path, use_audio=False):
    ...
    if use_audio == True:
        if which_dataset == "UCF":
            with open("splitting_scripts_main/mapping_audio_files_main_features_UCF.pickle", 'rb') as handle:
                list_h5 = pickle.load(handle)
        elif which_dataset == "ActivityNet":
            with open("splitting_scripts_main/mapping_audio_files_main_features_ActivityNet.pickle", 'rb') as handle:
                list_h5 = pickle.load(handle)
        elif which_dataset == "VGGSound":
            with open("splitting_scripts_main/mapping_audio_files_main_features_VGGSound.pickle", 'rb') as handle:
                list_h5 = pickle.load(handle)
    else:
        if which_dataset == "UCF":
            with open("splitting_scripts_main/mapping_videos_files_main_features_UCF.pickle", 'rb') as handle:
                list_h5 = pickle.load(handle)
        elif which_dataset == "ActivityNet":
            with open("splitting_scripts_main/mapping_videos_files_main_features_ActivityNet.pickle", 'rb') as handle:
                list_h5 = pickle.load(handle)
        elif which_dataset == "VGGSound":
            with open("splitting_scripts_main/mapping_videos_files_main_features_VGGSound.pickle", 'rb') as handle:
                list_h5 = pickle.load(handle)
...
def save_pickle_files(which_dataset, use_audio, root_path, original_dataset_path):
    ...
    for class_path, videos in tqdm(list_h5.items()):
        ...
        for video_item in videos:
            ...
            if use_audio == True:
                embedding_video = embedding_video[2].cpu().detach().numpy()
            else:
                embedding_video = embedding_video[0].cpu().detach().numpy()
