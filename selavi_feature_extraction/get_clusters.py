...
def get_cluster_assignments_gpu(
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):
        ...
        feat_v = torch.stack(f_v, dim=0)
        feat_a = torch.stack(f_a, dim=0)
        ...
        if args.distributed:
            ...
            # FIXME: Save videos individually
            out_path = os.path.join(args.output_dir, f'{args.exp_desc}')
            os.makedirs(out_path, exist_ok=True)
            save_path = os.path.join(out_path, f'{filename[0]}.pkl')
            save_data = [all_feat_v_list, all_feat_a_list, all_labels_list, all_filenames_list]
        ...
        #     for v, a, l, fn in zip(all_feat_v_list, all_feat_a_list, all_labels_list, all_filenames_list):
        #         all_feat_v.append(v)
        #         all_feat_a.append(a)
        #         all_labels.append(l)
        ...
        #     else:
        #         K = feat_v.size(0)
        #     PS_v = torch.zeros((N, T, K), dtype=dtype, device=device)
        #     PS_a = torch.zeros((N, T, K), dtype=dtype, device=device)
        ...
        #     # fr = to
        #     for v, a, l, fn in zip(all_feat_v, all_feat_a, all_labels, all_filenames):
        #         all_data.append([v, l, a, fn])
    ...
    if args.rank == 0:
        ...
        # FIXME: changed to save videos individually
...
def parse_args():
    ...
    # own arguments
    parser.add_argument("--overwrite", type='bool', default='False', help="Overwrite existing features")
    parser.add_argument("--num_shards", default=20, type=int)
    parser.add_argument("--run", default=-1, type=int,
                        help="Number of runs (out of num_shards) for parallel evaluation.")
...
if __name__ == '__main__':
    ...
    dataset = AVideoDataset(
        ...
        output_path=os.path.join(args.output_dir, f'{args.exp_desc}'), # TODO: for filtering existing features
    )
