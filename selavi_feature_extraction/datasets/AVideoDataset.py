...
class AVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.output_path = output_path # for filtering existing features
    ...
    def _construct_loader(self):
        ...
        if not self.overwrite:
            # Filter by already existing features (ActivityNet, VGGSound) (assumes features to be stored individually)
            # FIXME:
            valid_videos = [os.path.splitext(os.path.basename(self._path_to_videos[i]))[0] for i in self.valid_indices]
            self.output_path
            valid_ids = []
            existing_files = [os.path.splitext(os.path.basename(file))[0] for file in glob.glob(os.path.join(self.output_path, "*"))]
            for i, valid_vid in zip(self.valid_indices, valid_videos):
                if valid_vid not in existing_files:
                    valid_ids.append(i)
            self.valid_indices = valid_ids
            print(f"Filtering for already existing features: Valid videos: {len(self.valid_indices)}", flush=True)
        ...
        # self.valid_indices = self.manager.list([669,1103,1114,1115,1117,1118,1145,1808,1891,1894,1908,1930,3192,3218,3331,3795,3871,10901,10961,11221])
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["train", "val"] and self.dual_data:
            ...
            num_clips = int(frames_length / fps) # FIXME: - 1
