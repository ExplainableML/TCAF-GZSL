...
from src.utils import get_best_evaluation, evaluate_dataset_baseline
...
class MeanClassAccuracy(Metric):
    def __init__(self, model, dataset, device, distance_fn,new_model_sequence, args):
        super(MeanClassAccuracy, self).__init__()
        self.model = model
        self.new_model_sequence=new_model_sequence
    ...
    def __call__(self, outputs, target, loss_outputs):
        ...
        evaluation = evaluate_dataset_baseline(dataset_tuple=self.dataset, model=self.model, device=self.device,
                                               distance_fn=self.distance_fn,
                                               new_model_sequence=self.new_model_sequence,
                                               args=self.args)
