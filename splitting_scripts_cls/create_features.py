...
import os
import cv2
import utils_create_features
import json
which_dataset="UCF"
use_audio=False

dictionary_pkl, list_h5=utils_create_features.create_dict_features(which_dataset=which_dataset, use_audio=use_audio)
number_of_videos=0
save_root_path = "/mnt/supervised_unaveraged_fixed"
for name in tqdm(list_h5):
        with h5py.File(name, "r") as f:
            ...
            for index in range(len(f[b_group_key])):
                    ...
                    if use_audio==True:
                        features_from_dict=all_features_dict[2]
                    else:
                        features_from_dict=all_features_dict[0]
